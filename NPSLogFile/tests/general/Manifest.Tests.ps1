Describe "Validating the module manifest" {
    ...
    $manifest = ((Get-Content "$moduleRoot\NPSLogFile.psd1") -join "`n") | Invoke-Expression
    [version]$moduleVersion = Get-Item "$moduleRoot\NPSLogFile.psm1" | Select-String -Pattern '\$script:PSModuleVersion = "(.*?)"' | ForEach-Object { $_.Matches[0].Groups[1].Value }
